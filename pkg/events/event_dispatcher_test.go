...
// TearDownTest is a function that will be called after each test.
...
func (suite *EventDispatcherTestSuite) TestEventDispatcher_Has() {
	...
	suite.Nil(err)
	suite.Equal(2, len(suite.eventDispatcher.handlers[suite.event.GetName()]))
	has, index := suite.eventDispatcher.Has(suite.event.GetName(), &suite.handler)
	assert.True(suite.T(), has)
	assert.Equal(suite.T(), 0, index)

	has, index = suite.eventDispatcher.Has(suite.event.GetName(), &suite.handler2)
	assert.True(suite.T(), has)
	assert.Equal(suite.T(), 1, index)

	has, index = suite.eventDispatcher.Has(suite.event.GetName(), &suite.handler3)
